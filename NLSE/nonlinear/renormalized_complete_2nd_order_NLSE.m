function nonlin=renormalized_complete_2nd_order_NLSE(u,t,simulation_params)
%
...
%compute t-model term
    [nonlin1,uuu_tilde] = tmodel_term_NLSE(u_full,nonlin0,kappa,F_modes);
    
%compute t^2 term
    [nonlin2, ~, ~] = t2model_term_NLSE(u_full,nonlin0,uuu_tilde,kappa,F_modes,G_modes,k);

%compute nonlinear part of right hand side of t-model for NLSE
if simulation_params.time_dependence == 1
    
    nonlin = nonlin0([1:N+1,2*M-N+1:2*M]) + t*coeffs(1)*nonlin1([1:N+1,2*M-N+1:2*M]) + t^2*coeffs(2)*nonlin2([1:N+1,2*M-N+1:2*M]);
    
elseif simulation_params.time_dependence == 0
    
    nonlin = nonlin0([1:N+1,2*M-N+1:2*M]) + coeffs(1)*nonlin1([1:N+1,2*M-N+1:2*M]) + coeffs(2)*nonlin2([1:N+1,2*M-N+1:2*M]);%1:2*N+1
